...
const fileDict: Record<string, { mobile?: string; tablet?: string; desktop?: string; original: string }> = {};
...
Object.keys(allFiles).forEach(path => {
  ...
  if (match) {
    ...
    if (size === '400') fileDict[base].mobile = `/images/gallery/${filename}`;
    if (size === '768') fileDict[base].tablet = `/images/gallery/${filename}`;
    if (size === '1200') fileDict[base].desktop = `/images/gallery/${filename}`;
  } else {
...
export const galleryImages: Image[] = Object.values(fileDict).map(item => {
